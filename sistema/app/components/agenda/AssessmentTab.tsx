...
export function AssessmentTab({
  ...
  const visibleRows = useMemo(
    () =>
      rows.filter(
        (row) => courseFilter === "all" || row.course.id === courseFilter,
      ),
    [courseFilter, rows],
  );
  ...
  const summary = useMemo(() => {
    ...
    for (const row of visibleRows) {
      const cell = row.cells.find(
        (item) => item.assessmentId === selectedWork?.id,
      );
      ...
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div className="grid min-w-0 flex-1 gap-3 sm:grid-cols-[minmax(15rem,1fr)_11rem]">
          <div className="grid gap-1.5">
            ...
            >
              {works.length === 0 ? (
                <option value="">{t("agenda.noPracticos")}</option>
              ) : null}
              {works.map((work) => (
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {selectedWork ? (
            ...
            >
              ...
              {showManage ? t("agenda.close") : t("agenda.manageWork")}
            </Button>
          ) : null}
          <Button onClick={() => setShowCreate((open) => !open)}>
            ...
            {showCreate ? t("agenda.cancel") : t("agenda.newPractico")}
          </Button>
        </div>
      </div>
        ...
        <>
          <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            ...
            <p
              ...
              aria-live="polite"
            >
              {t("agenda.workSummary", {
                pending: summary.pendiente,
                submitted: summary.presentado,
                approved: summary.aprobado,
                failed: summary.desaprobado,
              })}
            </p>
          </div>
              ...
              <thead className="bg-muted/45 text-left">
                <tr>
                  ...
                  {selectedWork.graded ? (
                    <th
                      scope="col"
                      className="w-28 px-3 py-2.5 text-center font-medium"
                    >
                      {t("agenda.grade")}
                    </th>
                  ) : null}
                      ...
                      {selectedWork.graded ? (
                        <td className="px-3 py-2 text-center">
                          <Input
                            ...
                            aria-label={t("agenda.scoreOf", { name: fullName })}
                            onBlur={(event) =>
                              saveScore(row, event.target.value)
                            }
                            onKeyDown={(event) => {
                              if (event.key === "Enter")
