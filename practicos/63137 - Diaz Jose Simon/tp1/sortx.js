...
function parseType(type) {
    ...
    if (type !== TYPE_ALPHA) throw new Error('Tipo inválido: use "alpha" o "num"');
    return false;
}

function parseOrder(order) {
    if (order === ORDER_DESC) return true;
    if (order !== ORDER_ASC) throw new Error('Orden inválido: use "asc" o "desc"');
    return false;
}

function parseByValue(spec) {
    const parts = spec.split(':');
    if (parts.length > MAX_SPEC_PARTS) throw new Error('Formato inválido: use campo[:tipo[:orden]]');
    return {
        name: parts[NAME_INDEX],
        numeric: parts.length > TYPE_INDEX ? parseType(parts[TYPE_INDEX].toLowerCase()) : false,
        descending: parts.length > ORDER_INDEX ? parseOrder(parts[ORDER_INDEX].toLowerCase()) : false
    };
}

function parseDelimiterValue(delim) {
    if (delim === ESCAPED_TAB) return TAB;
    if (delim.length !== 1) throw new Error('El delimitador debe ser un único carácter');
    return delim;
}
...
function parseArgs(argv) {
    ...
    if (!hasBy) {
        throw new Error('Debe especificar al menos un criterio de ordenamiento con --by');
    }
