...
function editorCSV(data, columns) {
        ...
        <Box flexDirection="column">
            <Box>
                <Box width={anchoIndice + 2}>
                    <Text color={ESTILOS.titulo}>#</Text>
                </Box>
                {columns.map((col, i) => (
                    <Box key={col} width={anchos[i] + 2} backgroundColor={esColumnaActiva(i) ? ESTILOS.acentoDim : undefined}>
                        <Text color={esColumnaActiva(i) ? ESTILOS.acento : ESTILOS.titulo}>{col.toUpperCase()}</Text>
                    </Box>
                ))}
            </Box>
            {slicedData.map((fila, filaIdx) => (
                <Box flexShrink={0} key={filaIdx}>
                    <Box width={anchoIndice + 2} backgroundColor={esFilaActiva(inicio + filaIdx) ? ESTILOS.acentoDim : undefined}>
                        <Text color={esFilaActiva(inicio + filaIdx) ? ESTILOS.acento : ESTILOS.titulo}>{inicio + filaIdx + 1}</Text>
                    </Box>
                    {columns.map((col, i) => (
                        <Box key={col} width={anchos[i] + 2} backgroundColor={esCeldaActiva(inicio + filaIdx, i) ? "white" : undefined}>
                            <Text color={esCeldaActiva(inicio + filaIdx, i) ? "black" : ESTILOS.secundario}>{String(fila[col])}</Text>
                        </Box>
                    ))}
                </Box>
            ))}
...
function BarraAtajos({modo, dataBool, seleccionado}) {
    const opciones = ATAJOS.filter((a) => a.tipo.includes(modo) && (dataBool || a.showOnEmpty));
    return (
        <Box paddingTop={1} justifyContent="space-between">
            <Box>
                {opciones.map((a, i) => (
                    <Text key={i} color={ESTILOS.secundario}>
                        <Text bold color={ESTILOS.acento}>{a.tecla}</Text> {a.funcion}{i < opciones.length - 1 ? " · " : ""}
                    </Text>
                ))}
            </Box>
            {dataBool && (
...
function App({ruta: rutaInicial, data: dataInicial, columns: columnsInicial, errorInicial}) {
    const {exit} = useApp();
    const {columnas, filas} = useTamanoPantalla();
    ...
    const confirmarGuardado = async (nombre) => {
        try {
            await writeFile(nombre, editorCSV(data, columns), {encoding: "utf-8"});
            setRuta(nombre);
            setError(null);
        } catch (e) {
    ...
    return (
        <Box width={columnas} height={filas} alignItems="stretch" flexDirection="column" borderStyle="round" borderColor={ESTILOS.borde} backgroundColor={ESTILOS.fondo} paddingX={1}>
            ...
            <BarraAtajos modo={modo} dataBool={hayDatos} seleccionado={seleccionado} />
        </Box>
    );
}
...
async function init() {
    ...
    if (rutaArchivo) {
        try {
            data = await leerCSV(rutaArchivo);
            ruta = rutaArchivo;
        } catch (error) {
