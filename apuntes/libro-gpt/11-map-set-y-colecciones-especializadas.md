...
Construí un índice de productos con un `Map` por código, un `Set` de categorías y un `Map` de frecuencias por marca. Detectá códigos duplicados, conservá una política explícita sobre cuál gana y generá un resumen JSON convirtiendo las colecciones a arrays u objetos de manera controlada. Agregá una caché `WeakMap` de cálculos costosos asociados a cada objeto producto.
