...
Modelá un expediente con datos personales, domicilio, materias y metadatos. Implementá una actualización inmutable del domicilio y otra de una materia, conservando referencias de las ramas no modificadas. Comprobá con `===` qué niveles se compartieron y cuáles se reemplazaron. Serializá a JSON y enumerá la información que no podría conservarse si agregás métodos, símbolos o `undefined`.
