...
Creá una `ColeccionDeNotas` que:

1. mantenga su array en un campo privado;
2. sea iterable sin exponer el array original;
3. tenga una etiqueta `ColeccionDeNotas`;
4. use un símbolo local para guardar metadatos de versión;
5. ofrezca un iterador asincrónico que entregue una nota por vez con una demora simulada.

Probá `for...of`, spread, `for await...of`, `Reflect.ownKeys` y `JSON.stringify`, y explicá por qué cada operación ve propiedades distintas.
